...
int main (void)
{
	...
	/* Clear BSS. */
	bss_init (); /* BSS: 프로그램 실행 전에 초기화해야 하는 데이터를 저장하는 공간
					BSS를 초기화한다. */
	...
	serial_init_queue (); // 시리얼 통신을 위한 큐 초기화
	timer_calibrate (); // 부팅되면서 해당 하드웨어의 1 tick이 얼마인지 계산
	...
	run_actions (argv); // 커맨드라인에서 전달된 인자를 기반으로 특정 작업 수행
						// 예를 들어, 인자가 mfa면 mfa.c에 있는 run_mfqtest함수 호출
